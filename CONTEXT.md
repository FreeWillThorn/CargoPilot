...
**订单智能体**:
A dedicated Admin-only Workflow Section where an Admin User gives natural-language goals for one selected Import Order, then the agent understands the task, plans controlled steps, uses approved order tools, and produces data-entry applications or risk findings for administrator confirmation. Its MVP focuses on source data intake without Task Templates and order risk prompts.
_Avoid_: AI资料收集箱 alias, autonomous order operator, unrestricted database agent

**Order Agent Conversation Draft**:
A retained order-intelligence workspace that stores the Admin User's chat messages, uploaded source summaries, missing-field questions, and proposed order or Goods Line drafts until the user supplies enough information or discards the conversation.
_Avoid_: stateless prompt, lost upload attempt, automatic order creation

**Agent Processing Trace**:
A user-visible timeline of each meaningful Order Agent processing step, including local attachment parsing, model calls, model success or failure, selected agents, generated outputs, and blocked steps.
_Avoid_: hidden spinner, fake reasoning stream, undisclosed local preprocessing

**Order Creation Draft**:
A proposed new Import Order plus optional Goods Line candidates generated by `订单智能体` when no Import Order is selected. It is not a system Import Order until an Admin User confirms it.
_Avoid_: auto-created order, direct import

**Order Update Draft**:
A proposed change to the selected Import Order or its Goods Lines generated by `订单智能体` when an Import Order is selected. It must pass administrator confirmation before it changes system records.
_Avoid_: direct edit, silent overwrite

**Master Data Draft**:
A proposed Supplier or Consignee record generated by `订单智能体` from user-provided business material. It must show possible duplicate risk and cannot become master data until an Admin User confirms it.
_Avoid_: silent supplier creation, automatic customer creation, duplicate master data

**Minimal Goods Line Draft**:
A proposed Goods Line generated by `订单智能体` when at least a Chinese or English goods name is known. Missing fields remain empty and are shown as missing information instead of blocking the draft.
_Avoid_: requiring complete product data, invented package or price fields

**Task Understanding Agent**:
The `订单智能体` model agent that reads the Admin User's natural-language input, attachment summaries, and current Import Order context, then decides the user's goal, required specialist agents, missing information, and next plan.
_Avoid_: fixed task-template picker, direct executor

**Data Entry Agent**:
The `订单智能体` model agent that turns the conversation, source summaries, and optional current Import Order context into Order Creation Drafts, Order Update Drafts, Master Data Drafts, Minimal Goods Line Drafts, and missing-field questions.
_Avoid_: local-only importer, direct database writer

**Order Risk Agent**:
The `订单智能体` model agent that uses the current order context, goods, supplier grouping, destination/origin information, and source summaries to identify possible customs, document, compliance, or workflow risks and suggest business actions.
_Avoid_: local keyword checker, generic risk score
