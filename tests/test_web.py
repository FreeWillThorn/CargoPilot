...
class WebShellTest(unittest.TestCase):
    ...
    def test_order_agent_empty_state_and_retained_conversation(self):
        ...
        self.assertIn("order-agent-main", page)
        self.assertIn("order-agent-workspace-scroll", page)
        self.assertIn("order-agent-results", page)
    ...
    def test_action_drawer_is_closeable_overlay(self):
        ...
        self.assertIn(".order-agent-conversation-scroll { max-height:calc(100dvh - 250px); overflow:auto; }", css)
        self.assertIn(".order-agent-main { min-width:0; display:grid; gap:16px; }", css)
        self.assertIn(".order-agent-workspace-scroll { max-height:calc(100dvh - 330px); min-height:360px; overflow:auto; }", css)
        self.assertIn(".order-agent-results { border-left:3px solid var(--accent); }", css)
